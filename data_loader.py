...
class InputExample(object):
    ...
    def __repr__(self):
        return str(self.to_json.string())
...
class REPorcessor(object):
    """Processor for the Relational Extraction"""
...
def convert_examples_to_features(examples, max_seq_len, tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # The mask ha 1 for real tokens and 0 for padding toknes. Only real tokens are attended to.
        ...
        assert len(token_type_ids) == max_seq_len, "Error with input token type length {} vs {}".format(len(token_type_ids), max_seq_len)
        ...
        if ex_index < 5:
            ...
            logger.info("toknes: %s" % " ".join([str(x) for x in tokens]))
...
def load_and_cache_examples(args, tokenizer, mode):
    precessor = processors[args.task](args)
    ...
    if os.path.exists(cached_features_file):
        ...
        logger.info("Create features from dataset file at %s", args.data_dir)
        if mode == "train":
            examples = precessor.get_examples("train")
        elif mode == "dev":
            examples = precessor.get_examples("dev")
        elif mode == "test":
            examples = precessor.get_examples("test")
        else:
